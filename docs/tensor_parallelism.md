...
| HiDream O1 | `HiDreamO1Transformer` | `language_model.layers`, `visual.blocks` (LLM GQA 32/8; vision packed `attn.qkv`; embeddings / merger stay) |
...
| Krea2 | `SingleStreamDiT` | `blocks` (unfused GQA `wq`/`wk`/`wv`; attention `gate` colwise; `wo` rowwise; `txtfusion` / `tproj` stay) |
| MageFlow | `MageFlowTransformer2DModel` | `transformer_blocks` (QwenImage-style unfused `to_q`/`to_k`/`to_v`; `img_mod.1` excluded) |
| Hunyuan3D | `HunYuanDiTPlain` | `blocks` (unfused `to_q`/`to_k`/`to_v`; dense `fc1`/`fc2`; MoE last 6 layers stay; `skip_linear` excluded) |
| Hunyuan3D v2 | `Hunyuan3Dv2` | `double_blocks`, `single_blocks` (Flux packed double-stream QKV; fused `linear1` stays). Not a Flux subclass. |
| Kandinsky 5 | `Kandinsky5` | `text_transformer_blocks`, `visual_transformer_blocks` (unfused `to_query`; attn/FF `out_layer` rowwise; `modulation.out_layer` stays) |
| Ernie Image | `ErnieImageModel` | `layers` (unfused `to_q`; SwiGLU `gate_proj`/`up_proj` colwise; `linear_fc2` rowwise; root `adaLN_modulation` stays) |
| TripoSplat | `LatentSeqMMFlowModel` | `noise_refiner` / `context_refiner` / `blocks` / `*_repo_layers` (packed `attn.qkv`; `attn.out` rowwise; RePo `final_map` colwise; `gate_map`/`content_map` stay; MultiHeadRMSNorm `gamma` sliced) |
| CogVideoX | `CogVideoXTransformer3DModel` | `blocks` (unfused `q`/`k`/`v`; `attn_out`/`ff_out` rowwise; `ff_proj` colwise; adaLN `norm{1,2}.linear` stay) |
| SeedVR2 | `NaDiT` | `blocks` (packed MMModule `proj_qkv.{vid,txt,all}`; `proj_out` rowwise; MLP `proj_in`/`proj_in_gate` colwise; AdaSingle is Parameters) |
| AuraFlow | `MMDiT` | `double_layers`, `single_layers` (unfused `w1q`/`w2q`; `w1o`/`w2o` rowwise; `c_fc1`/`c_fc2` colwise; `modC`/`modX`/`modCX` stay). SD3 uses `OpenAISignatureMMDITWrapper` and is not this row. |

Head-split models (heads divided by world_size): QwenImage, MiniTrainDIT, WanModel (and CausalWan via MRO), HiDreamImageTransformer2DModel, HiDream O1 (`HiDreamO1Transformer`; LLM `num_heads`/`num_kv_heads` under `language_model.layers`, vision packed QKV under `visual.blocks`; mixed 32 and 16, each Attention divides independently), ACE-Step 1.0/1.5, MiniMax H3, Flux / HunyuanVideo / Chroma / Hunyuan3Dv2 (double-stream only; `SingleStreamBlock.num_heads` stays full because fused `linear1` is unreplicated; `ChromaRadiance` inherits Chroma), SD3 / MMDiT, Ideogram 4, JoyImage, Lens, NextDiT (`n_local_heads` / `n_local_kv_heads`; root `n_heads` stays), PixelDiT (`MMDiTJointAttention` / `RotaryAttention`; `PiTBlock.num_heads` stays for RoPE dim), Mochi (`AsymmetricAttention`; root `num_heads` stays, `pos_frequencies` is sliced), HunyuanDiT (`Attention` / `CrossAttention`; root `num_heads` stays), PixArt (`AttentionKVCompress` / `MultiHeadCrossAttention`; root `num_heads` stays; KV-compress and full-dim QK-norm blocks keep full heads), Stable Audio (`AudioDiffusionTransformer` self/cross attn; RoPE is per-`dim_heads`, conformer stays), Boogu (`BooguTransformer2DModel`; Q heads split, `kv_heads` stays when not divisible), Krea2 (`SingleStreamDiT`; GQA `heads`/`kvheads` split under `blocks`; root `heads` stays), MageFlow (`MageFlowTransformer2DModel`; QwenImage-style unfused attention), Hunyuan3D (`HunYuanDiTPlain`; unfused self/cross attn; root `num_heads` stays; MoE unreplicated), Kandinsky 5 (`Kandinsky5`; unfused `to_query`; 28 heads; RoPE along `head_dim`; modulation stays), Ernie Image (`ErnieImageModel`; unfused `to_q`; 32 heads; RoPE along `head_dim`; root `num_heads` stays), TripoSplat (`LatentSeqMMFlowModel`; packed `attn.qkv`; RePo `num_heads` follows shard; MultiHeadRMSNorm `gamma` sliced; root `num_heads` stays), CogVideoX (`CogVideoXTransformer3DModel`; unfused joint QKV; root `num_attention_heads` stays; RoPE along `head_dim`), SeedVR2 (`NaDiT`; packed MMModule QKV; 3B=20 heads / 7B=24 heads), AuraFlow (`MMDiT` concrete class; `n_heads` on Double/SingleAttention; SD3 wrapper is a different allowlist key).
...
HiDream O1 is supported. Production is 32 Q / 8 KV on the LLM (both 2-GPU-divisible) and 16 packed vision heads. Prefixes `language_model.layers` and `visual.blocks` shard those attentions; `embed_tokens`, `x_embedder`, `t_embedder1`, vision `patch_embed`/`merger`, and `final_layer2` stay full hidden width — the same pattern as Flux `img_in`. Per-head Gemma3 QK-norm stays. OmniGen2 production 21/7 still does not divide and stays off the allowlist.
...
- **LoRA**: DynamicVRAM attaches LowVramPatch to ParallelLinear.weight_function. Packed colwise layers (Flux/Hunyuan/Chroma `*.qkv`, MiniMax `qkv_proj`/`fc1`, NextDiT GQA `pack_sizes`) slice LoRA diffs per pack — a naive row cut would mix Q/K/V. Flux+LoRA verified on unreplicated QKV; packed QKV LoRA is unit-tested (equal packs and GQA `pack_sizes`). After `parallelize_model`, unfused Qwen `to_q`, Wan `q`, Kandinsky `to_query`, MiniTrainDIT/Anima `q_proj`, and HiDreamO1 vision packed `attn.qkv` LoRA diffs follow the shard (rank-1 unfused is not a naive prefix cut). Qwen/Wan LoRA live e2e still needs the 2-GPU bounce. DoRA/LoHa/OFT under TP are not yet fully supported.
...
- **HiDream O1 (`HiDreamO1Transformer`)**: LLM GQA 32/8 under `language_model.layers` (`q_proj`/`k_proj`/`v_proj` colwise, `o_proj` rowwise; `num_kv_heads` divided independently). Vision packed `visual.blocks.*.attn.qkv` (`[Q|K|V]`, 16 heads) with `.proj` rowwise; `.attn.qkv` is un-excluded at runtime (SD3 suffix collision). `embed_tokens`, `x_embedder`, `t_embedder1`, vision `patch_embed`/`merger`, and `final_layer2` stay — full hidden width into sharded attention. Per-head Gemma3 QK RMSNorm stays. Mixed 32 and 16 head counts: each Attention divides independently; leftover `visual.num_heads` is outside `visual.blocks` and stays.
...
- **Krea2 (`SingleStreamDiT`)**: Unfused GQA 48 Q / 12 KV — both divide by 2-GPU world_size, so Q and KV shard. Attention `gate` is `Linear(dim, dim)` and must be colwise so `out * sigmoid(gate)` stays local-dim after head-split. `wo` is rowwise (`endswith(".wo")` — Wan's `.o` suffix does not match). SwiGLU `gate`/`up` colwise, `down` rowwise. Prefix `blocks` only — `txtfusion` (txtheads=20, not 2-GPU-divisible) and `tproj` (6-way chunk) stay replicated. Root constructor `heads` is not divided; per-block `Attention.heads`/`kvheads` are. QKNorm is per-`headdim`.
- **MageFlow (`MageFlowTransformer2DModel`)**: Not an MRO subclass of `QwenImageTransformer2DModel`. 12× `QwenImageTransformerBlock` under `transformer_blocks`. Same unfused Qwen Image attention path (`to_q`/`to_k`/`to_v`, 24 heads). `img_mod.1` / `txt_mod.1` stay (global 6-way chunk).
...
- **Flux / HunyuanVideo / Chroma / Hunyuan3Dv2**: Packed colwise on double-stream `img_attn.qkv` / `txt_attn.qkv` (`[Q|K|V]`); `*.proj` is rowwise. Single-stream fused `linear1`/`linear2` stay replicated (QKV+MLP unequal packs). `SingleStreamBlock.num_heads` is not divided. Hunyuan `txt_in` TokenRefiner is outside TP prefixes and stays full-width. Hunyuan3Dv2 is not a Flux subclass — it is allowlisted by concrete class with the same prefixes. `ChromaRadiance` inherits Chroma via MRO.
...
- **HunyuanDiT (`HunYuanDiT`)**: Packed colwise on self-attn `Wqkv` (`[Q|K|V]`) and cross-attn `kv_proj` (`[K|V]`, `pack_count=2`); `q_proj`/`out_proj` follow colwise/rowwise; MLP `fc1`/`fc2` shard. `default_modulation` and `skip_linear` stay (full-width shift / 2×hidden cat). Root `num_heads` stays.
- **Hunyuan3D (`HunYuanDiTPlain`)**: Different class from hydit `HunYuanDiT`. Unfused `to_q`/`to_k`/`to_v` (16 heads, 2-GPU-divisible) with per-head QK-norm. Dense MLP `fc1`/`fc2` shards. Last 6 layers are `MoEBlock` — experts, shared FF, and gate stay replicated (`.moe.` subtree). `skip_linear` stays (2×hidden cat). Prefix `blocks` so `x_embedder` / `t_embedder` / `final_layer` stay. Root `num_heads` is constructor metadata and is not divided.
- **Hunyuan3D v2 (`Hunyuan3Dv2`)**: Flux `DoubleStreamBlock` / `SingleStreamBlock` (16 heads, 2-GPU-divisible), not a Flux subclass. Same packed double-stream QKV path. Prefixes keep `latent_in` / `time_in` / `cond_in` / `final_layer` unreplicated.
- **Kandinsky 5 (`Kandinsky5`)**: Unfused `to_query`/`to_key`/`to_value` (28 heads from `model_dim=1792` / `axes_dims` sum 64, 2-GPU-divisible). Attn and FF `out_layer` are rowwise; `in_layer` is colwise. `text_modulation.out_layer` / `visual_modulation.out_layer` stay (6-way / 9-way chunk on full dim). Prefixes keep root `OutLayer` and embeddings unreplicated. Per-head RMSNorm; RoPE (`apply_rope1`) along `head_dim`.
- **Ernie Image (`ErnieImageModel`)**: Unfused `to_q`/`to_k`/`to_v` (32 heads, 2-GPU-divisible). `to_out.0` is rowwise. SwiGLU `gate_proj`/`up_proj` colwise, `linear_fc2` rowwise (`fc2` keyword). Prefix `layers` so `x_embedder` / `text_proj` / time embeddings / root `adaLN_modulation` (6-way chunk) / `final_norm` / `final_linear` stay. Per-head RMSNorm; RoPE (`apply_rope_split_half`) along `head_dim`. Root `num_heads` is constructor metadata and is not divided.
- **TripoSplat (`LatentSeqMMFlowModel`)**: Packed colwise on `attn.qkv` (`reshape(..., 3, heads, head_dim)`). `.attn.qkv` is un-excluded at runtime because it collides with the SD3 suffix. `attn.out` and MLP `mlp.mlp.2` are rowwise. RePo3D `final_map` is naive colwise (`3 * num_heads` laid out as `(heads, 3)` so a head split is a contiguous cut). `gate_map`/`content_map` stay (bottleneck into `final_map`). Prefixes include `*_repo_layers` so RePo `num_heads` follows the shard. `MultiHeadRMSNorm.gamma` (`heads, head_dim`) is sliced. Root `adaLN_modulation` / `input_layer` / `out_layer` stay. Default 1024/64 = 16 heads, 2-GPU-divisible. Root `num_heads` stays.
- **CogVideoX (`CogVideoXTransformer3DModel`)**: Unfused joint self-attn `q`/`k`/`v` (30 heads, 2-GPU-divisible). `attn_out` and `ff_out` are rowwise; `ff_proj` is colwise (generic `proj` would otherwise make the up-projection rowwise). Prefix `blocks` so `patch_embed` / time embeddings / `proj_out` stay. `CogVideoXLayerNormZero.linear` (`norm1.linear` / `norm2.linear`) stays — 6-way chunk on full dim. Per-head QK LayerNorm is along `head_dim`. RoPE (`apply_rotary_emb`) is interleaved pairs along `head_dim`. Root `num_attention_heads` is constructor metadata and is not divided.
- **SeedVR2 (`NaDiT`)**: Packed colwise on MMModule `proj_qkv.{vid,txt,all}` (`reshape(..., 3, heads, head_dim)`). `proj_out` is rowwise. MLP `proj_in` / SwiGLU `proj_in_gate`+`proj_in` are colwise (generic `proj` would invert them). Prefix `blocks` so `vid_in` / `txt_in` / `emb_in` / `vid_out` stay. AdaSingle is Parameters, not Linears. QK RMSNorm is per-`head_dim`. Production 3B is 20 heads, 7B is 24 — both 2-GPU-divisible. RoPE is along `head_dim` / partial `rope_dim`. `_resolve_q_proj` unwraps MMModule `.vid`/`.all` so leftover `heads` divide.
- **AuraFlow (`MMDiT`)**: Same Python class name as SD3's parent. SD3's inner model is `OpenAISignatureMMDITWrapper` and matches that allowlist key first (`joint_blocks`, packed `attn.qkv`). Aura's concrete class is `MMDiT` with prefixes `double_layers` / `single_layers`. Unfused `w1q`/`w1k`/`w1v` / `w2q`/`w2k`/`w2v` are colwise; `w1o`/`w2o` are rowwise (`w1` is a colwise keyword and `w2` is Megatron down-proj — both would mis-mode without explicit suffixes). SwiGLU `c_fc1`/`c_fc2` colwise (`fc2` is otherwise rowwise), `c_proj` rowwise. `modC.1` / `modX.1` / `modCX.1` stay (6-way chunk). `init_x_linear` / `final_linear` are outside TP prefixes. Default QK-norm is per-`head_dim` (`mh_qknorm=False`). 12 heads, 2-GPU-divisible.
